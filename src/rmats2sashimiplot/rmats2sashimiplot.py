...
def checkout(parser, options):
    ...
    if ((options.s1 is None and options.b1 is None and options.s2 is None
         and options.b2 is None)):
    ...
    if (options.events_file is None
            or options.event_type is None) and options.coordinate is None:
...
def conf_setting_file(options,
                      gene_no_str=None,
                      gene_symbol=None,
                      events_name_level=None,
                      id_str=None):
    """
    ...
    if gene_no_str is not None:
        setting_file = open(
            os.path.join(options.out_dir, "Sashimi_index_" + gene_no_str,
                         "sashimi_plot_settings.txt"), 'w')
    else:
        setting_file = open(
            os.path.join(options.sashimi_path, "sashimi_plot_settings.txt"),
            'w')
    ...
    if gene_no_str is None:  # the case with coordinate
        for rr in range(0, len_sample1):
            sample_labels_arr1.append('\"{0}-{1}\"'.format(
                options.l1, str(rr + 1)))
        for rr in range(0, len_sample2):
            sample_labels_arr2.append('\"{0}-{1}\"'.format(
                options.l2, str(rr + 1)))
        ...
        for rr in range(0, len_sample1):
            try:
                ...
                warning_flag = True
            sample_labels_arr1.append('\"' + gene_symbol + ' ' + options.l1
                                      + '-' + str(rr + 1) + ' IncLevel: '
                                      + inc_1 + '\"')
        for rr in range(0, len_sample2):
            try:
                ...
                warning_flag = True
            sample_labels_arr2.append('\"' + gene_symbol + ' ' + options.l2
                                      + '-' + str(rr + 1) + ' IncLevel: '
                                      + inc_2 + '\"')
        if warning_flag:
            print("Warning: The inclusion levels of Event '{}' contains"
                  " 'NA' value, which could lead to unexpected output.".format(
                      id_str),
                  file=sys.stderr)
...
def plot_c(options, id_str):
    ...
    """
    path_index_gff = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
        'MISO/misopy/index_gff.py')
    path_sashimi_plot = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
    ...
    os.system("{} {} --index \"{}\" \"{}\"".format(python_executable,
                                                   path_index_gff, tmp_str,
                                                   options.sashimi_path))

    # call python sashimi_plot.py
    setting_str = os.path.join(options.sashimi_path,
    ...
    if options.group_info is not None:
        ...
        os.system("{} {} --plot-event \"{}\" \"{}\" \"{}\" "
                  "--output-dir {}".format(python_executable,
                                           path_sashimi_plot, id_str,
                                           options.sashimi_path, setting_str,
                                           output_path))
...
def plot_e(options, id_str, gene_symbol, events_no):
    ...
    out_index = os.path.join(
        options.out_dir, "Sashimi_index_" + gene_symbol + '_' + str(events_no))
    ...
    output_path = os.path.join(options.out_dir, "Sashimi_plot")
    print("{} {} --plot-event \"{}\" \"{}\" \"{}\" "
          "--output-dir {}".format(python_executable, path_sashimi_plot,
                                   id_str, out_index, setting_str,
                                   output_path))
    if options.group_info is not None:
        os.system("{} {} --plot-event \"{}\" \"{}\" \"{}\" "
                  "--output-dir {} --group-info {}".format(
...
def plot_with_coordinate(options):
    ...
    try:
        ...
        id_str = in_chr + "_" + in_coor_s + "_" + str(
            in_coor_e) + "_" + in_strand  # chr2_10101175_10104171_+
        gff3_file = tmp_str[4]
        fo = open(gff3_file, 'r')
        ...
        for line in fo:
            ...
            if is_mrna_or_transcript or item_type == "exon":
                ...
                if (in_strand == strand and
                    ((item_type == 'exon' and int(in_coor_s) <= int(coor_s)
                      and int(coor_e) <= int(in_coor_e)) or
                     (is_mrna_or_transcript and int(coor_s) < int(in_coor_e)
                    ...
                    if is_mrna_or_transcript:
                        ...
                        w1.write(
                            "%s\tensGene\t%s\t%s\t%s\t.\t%s\t.\tParent=%s;%s\n"
                            % (item_chr, item_type, coor_s, coor_e, strand,
                               id_str, annot_str))
                    if item_type == "exon":
...
class EventCoor(object):
    ...
    def generate_in_positive_order(self, seq_chr, gene_symbol, strand,
                                   event_type):
        if event_type == 'MXE':
            self.id_str = (seq_chr + "_" + self.up_s + "_" + self.up_e + "_"
                           + strand + "@" + seq_chr + "_" + self.e1st_s + "_"
                           + self.e1st_e + "_" + strand + "@" + seq_chr + "_"
                           + self.e2nd_s + "_" + self.e2nd_e + "_" + strand
                           + "@" + seq_chr + "_" + self.dn_s + "_" + self.dn_e
                           + "_" + strand)
        elif event_type == 'A5SS':
            self.id_str = (seq_chr + "_" + self.sh_s + "_" + self.sh_e + "_"
                           + strand + "@" + seq_chr + "_" + self.lo_s + "_"
                           + self.lo_e + "_" + strand + "@" + seq_chr + "_"
                           + self.fl_s + "_" + self.fl_e + "_" + strand)
        elif event_type == 'A3SS':
            self.id_str = (seq_chr + "_" + self.fl_s + "_" + self.fl_e + "_"
                           + strand + "@" + seq_chr + "_" + self.lo_s + "_"
                           + self.lo_e + "_" + strand + "@" + seq_chr + "_"
                           + self.sh_s + "_" + self.sh_e + "_" + strand)
        elif event_type == 'SE' or event_type == 'RI':
            self.id_str = (seq_chr + "_" + self.up_s + "_" + self.up_e + "_"
                           + strand + "@" + seq_chr + "_" + self.se_s + "_"
                           + self.se_e + "_" + strand + "@" + seq_chr + "_"
                           + self.dn_s + "_" + self.dn_e + "_" + strand)
    ...
    def generate_in_reversed_order(self, seq_chr, gene_symbol, strand,
                                   event_type):
        if event_type == 'MXE':
            self.id_str = (seq_chr + "_" + self.dn_s + "_" + self.dn_e + "_"
                           + strand + "@" + seq_chr + "_" + self.e2nd_s + "_"
                           + self.e2nd_e + "_" + strand + "@" + seq_chr + "_"
                           + self.e1st_s + "_" + self.e1st_e + "_" + strand
                           + "@" + seq_chr + "_" + self.up_s + "_" + self.up_e
                           + "_" + strand)
            ...
            self.id_str = (seq_chr + "_" + self.dn_s + "_" + self.dn_e + "_"
                           + strand + "@" + seq_chr + "_" + self.se_s + "_"
                           + self.se_e + "_" + strand + "@" + seq_chr + "_"
                           + self.up_s + "_" + self.up_e + "_" + strand)
        self.name_str = gene_symbol + "_" + self.id_str
...
def create_chr_aware_events_file(options):
    ...
    new_events_file_path = os.path.join(options.sashimi_path,
                                        'events_file.txt')
    if options.b1:
...
def plot_with_eventsfile(options):
    ...
    try:
        ...
        w2 = open(
            os.path.join(options.sashimi_path,
                         options.event_type + ".event.list.txt"), 'w')
        events_name_level = {}
        events_no = 0
        for line in fo:
            ...
            sashimi_path = os.path.join(
                options.out_dir,
                "Sashimi_index_" + gene_symbol + '_' + str(events_no))
            ...
            events_name_level[
                gene_symbol] = coor.inc_level1 + "_" + coor.inc_level2
            ...
            if strand == '+':
                ...
                if options.event_type == "SE":
                    w1.write("%s\tSE\tgene\t%s\t%s\t.\t%s\t.\tID=%s;Name=%s\n"
                             % (seq_chr, coor.up_s, coor.dn_e, strand,
                                coor.id_str, coor.name_str))
                    w1.write(
                        "%s\tSE\tmRNA\t%s\t%s\t.\t%s\t.\tID=%s.A;Parent=%s\n" %
                        (seq_chr, coor.up_s, coor.dn_e, strand, coor.id_str,
                         coor.id_str))
                    w1.write(
                        "%s\tSE\tmRNA\t%s\t%s\t.\t%s\t.\tID=%s.B;Parent=%s\n" %
                        (seq_chr, coor.up_s, coor.dn_e, strand, coor.id_str,
                         coor.id_str))
                    w1.write(
                        "%s\tSE\texon\t%s\t%s\t.\t%s\t.\tID=%s.A.up;Parent=%s.A\n"
                        % (seq_chr, coor.up_s, coor.up_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tSE\texon\t%s\t%s\t.\t%s\t.\tID=%s.A.se;Parent=%s.A\n"
                        % (seq_chr, coor.se_s, coor.se_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tSE\texon\t%s\t%s\t.\t%s\t.\tID=%s.A.dn;Parent=%s.A\n"
                        % (seq_chr, coor.dn_s, coor.dn_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tSE\texon\t%s\t%s\t.\t%s\t.\tID=%s.B.up;Parent=%s.B\n"
                        % (seq_chr, coor.up_s, coor.up_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tSE\texon\t%s\t%s\t.\t%s\t.\tID=%s.B.dn;Parent=%s.B\n"
                        % (seq_chr, coor.dn_s, coor.dn_e, strand, coor.id_str,
                           coor.id_str))
                    ...
                    w1.write(
                        ...
                        (seq_chr, coor.up_s, coor.dn_e, strand, coor.id_str,
                         coor.id_str))
                    w1.write(
                        "%s\tSE\texon\t%s\t%s\t.\t%s\t.\tID=%s.A.in;Parent=%s.A\n"
                        % (seq_chr, coor.se_s, coor.se_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tSE\texon\t%s\t%s\t.\t%s\t.\tID=%s.B.up;Parent=%s.B\n"
                    ...
                    w1.write("%s\tSE\tgene\t%s\t%s\t.\t%s\t.\tID=%s;Name=%s\n"
                             % (seq_chr, coor.fl_s, coor.sh_e, strand,
                                coor.id_str, coor.name_str))
                    w1.write(
                        "%s\tSE\tmRNA\t%s\t%s\t.\t%s\t.\tID=%s.A;Parent=%s\n" %
                        (seq_chr, coor.fl_s, coor.sh_e, strand, coor.id_str,
                         coor.id_str))
                    w1.write(
                        "%s\tSE\tmRNA\t%s\t%s\t.\t%s\t.\tID=%s.B;Parent=%s\n" %
                        (seq_chr, coor.fl_s, coor.sh_e, strand, coor.id_str,
                    ...
                    w1.write(
                        "%s\tSE\texon\t%s\t%s\t.\t%s\t.\tID=%s.A.up;Parent=%s.A\n"
                        % (seq_chr, coor.fl_s, coor.fl_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tSE\texon\t%s\t%s\t.\t%s\t.\tID=%s.A.dn;Parent=%s.A\n"
                        % (seq_chr, coor.lo_s, coor.lo_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tSE\texon\t%s\t%s\t.\t%s\t.\tID=%s.B.up;Parent=%s.B\n"
                        % (seq_chr, coor.fl_s, coor.fl_e, strand, coor.id_str,
                    ...
                    w1.write(
                        "%s\tSE\texon\t%s\t%s\t.\t%s\t.\tID=%s.B.dn;Parent=%s.B\n"
                        % (seq_chr, coor.sh_s, coor.sh_e, strand, coor.id_str,
                           coor.id_str))
                elif options.event_type == "A5SS":
                    w1.write("%s\tSE\tgene\t%s\t%s\t.\t%s\t.\tID=%s;Name=%s\n"
                             % (seq_chr, coor.sh_s, coor.fl_e, strand,
                                coor.id_str, coor.name_str))
                    w1.write(
                        "%s\tSE\tmRNA\t%s\t%s\t.\t%s\t.\tID=%s.A;Parent=%s\n" %
                        (seq_chr, coor.sh_s, coor.fl_e, strand, coor.id_str,
                         coor.id_str))
                    w1.write(
                        "%s\tSE\tmRNA\t%s\t%s\t.\t%s\t.\tID=%s.B;Parent=%s\n" %
                        (seq_chr, coor.sh_s, coor.fl_e, strand, coor.id_str,
                    ...
                    w1.write("%s\tMXE\tgene\t%s\t%s\t.\t%s\t.\tID=%s;Name=%s\n"
                             % (seq_chr, coor.up_s, coor.dn_e, strand,
                                coor.id_str, coor.name_str))
                    w1.write(
                        "%s\tMXE\tmRNA\t%s\t%s\t.\t%s\t.\tID=%s.A;Parent=%s\n"
                        % (seq_chr, coor.up_s, coor.dn_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tMXE\tmRNA\t%s\t%s\t.\t%s\t.\tID=%s.B;Parent=%s\n"
                        % (seq_chr, coor.up_s, coor.dn_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tMXE\texon\t%s\t%s\t.\t%s\t.\tID=%s.A.up;Parent=%s.A\n"
                        % (seq_chr, coor.up_s, coor.up_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tMXE\texon\t%s\t%s\t.\t%s\t.\tID=%s.A.1st;Parent=%s.A\n"
                        % (seq_chr, coor.e1st_s, coor.e1st_e, strand,
                           coor.id_str, coor.id_str))
                    w1.write(
                        "%s\tMXE\texon\t%s\t%s\t.\t%s\t.\tID=%s.A.dn;Parent=%s.A\n"
                        % (seq_chr, coor.dn_s, coor.dn_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tMXE\texon\t%s\t%s\t.\t%s\t.\tID=%s.B.up;Parent=%s.B\n"
                        % (seq_chr, coor.up_s, coor.up_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tMXE\texon\t%s\t%s\t.\t%s\t.\tID=%s.B.2nd;Parent=%s.B\n"
                        % (seq_chr, coor.e2nd_s, coor.e2nd_e, strand,
                           coor.id_str, coor.id_str))
                    w1.write(
                        "%s\tMXE\texon\t%s\t%s\t.\t%s\t.\tID=%s.B.dn;Parent=%s.B\n"
                        % (seq_chr, coor.dn_s, coor.dn_e, strand, coor.id_str,
                           coor.id_str))
                ...
                if options.event_type == "SE":
                    ...
                    w1.write(
                        ...
                        % (seq_chr, coor.up_s, coor.up_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tMXE\texon\t%s\t%s\t.\t%s\t.\tID=%s.A.2nd;Parent=%s.A\n"
                        % (seq_chr, coor.e2nd_s, coor.e2nd_e, strand,
                           coor.id_str, coor.id_str))
                    w1.write(
                        "%s\tMXE\texon\t%s\t%s\t.\t%s\t.\tID=%s.A.dn;Parent=%s.A\n"
                    ...
                    w1.write(
                        ...
                        % (seq_chr, coor.up_s, coor.up_e, strand, coor.id_str,
                           coor.id_str))
                    w1.write(
                        "%s\tMXE\texon\t%s\t%s\t.\t%s\t.\tID=%s.B.1st;Parent=%s.B\n"
                        % (seq_chr, coor.e1st_s, coor.e1st_e, strand,
                           coor.id_str, coor.id_str))
                    w1.write(
                        "%s\tMXE\texon\t%s\t%s\t.\t%s\t.\tID=%s.B.dn;Parent=%s.B\n"
            ...
            w1.close()
            try:
                conf_setting_file(options, gene_no_str, gene_symbol,
                                  events_name_level, coor.id_str)
            except Exception as e:
                print(e)
                print(
...
def main():
    ...
    label_group.add_argument("--l1",
                             dest="l1",
                             help="The label for the first sample.")
    label_group.add_argument("--l2",
                             dest="l2",
    ...
    rmats_group.add_argument(
        "-e",
        dest="events_file",
        help=("The rMATS output event file (Only if using " + rmats_group_str
              + ")"))
    ...
    group_sam.add_argument("--s1",
                           dest="s1",
                           help=sam_bam_sample_arg_desc_template.format(
                               num=1, kind='sam'))
    group_sam.add_argument("--s2",
                           dest="s2",
                           help=sam_bam_sample_arg_desc_template.format(
                               num=2, kind='sam'))
    ...
    group_bam.add_argument("--b1",
                           dest="b1",
                           help=sam_bam_sample_arg_desc_template.format(
                               num=1, kind='bam'))
    group_bam.add_argument("--b2",
                           dest="b2",
                           help=sam_bam_sample_arg_desc_template.format(
                               num=2, kind='bam'))
    ...
    optional_group.add_argument("--font-size",
                                dest="font_size",
                                default=8,
    ...
    optional_group.add_argument(
        ...
        help=('remove any leading "chr" from contig names in the provided'
              ' events file'))
    optional_group.add_argument("--ymax",
                                dest="ymax",
                                type=float,
