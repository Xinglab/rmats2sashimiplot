...
def print_reads_summary(reads, gene, paired_end=False):
    ...
    for n in range(num_isoforms):
        ...
        computed_const = True
        print("Iso %d (len = %d): %d unambiguous supporting reads" %
              (n, gene.isoforms[n].len, num_iso_reads))
    print("No. constitutive reads (consistent with all): %d" %
          (num_constitutive_reads))
...
def expected_read_summary(gene, true_psi, num_reads, read_len, overhang_len):
    ...
    psi_f = (true_psi * num_pos_iso1) / ((true_psi * num_pos_iso1) +
                                         ((1 - true_psi) * num_pos_iso2))
    ##
    ...
    p_NE = (1 - psi_f) * (
        (read_len + 1 -
         (2 * overhang_len)) / float(len(iso2_seq) - read_len + 1))
    # Compute probability of read supporting both:
    # p(NB) = p(isoform 1)p(NB read | isoform 1) + p(isoform 2)p(NB read | isoform 2)
    num_NB = (gene.get_part_by_label('A').len - read_len
              + 1) + (gene.get_part_by_label('C').len - read_len + 1)
    ...
    return [
        p_NI * num_reads, p_NE * num_reads, p_NB * num_reads,
        p_oh_violation * num_reads
    ]


def simulate_two_iso_reads(gene,
                           true_psi,
                           num_reads,
                           read_len,
                           overhang_len,
    ...
    if len(gene.isoforms) != 2:
        raise Exception(
            "simulate_two_iso_reads requires a gene with only two isoforms.")
    if len(true_psi) < 2:
        raise Exception(
            "Simulate reads requires a probability vector of size > 2.")
    reads_summary = [0, 0, 0]
    all_reads = []
    categories = []
    true_isoforms = []
    ...
    for k in range(0, num_reads):
        reads_sampled = sample_random_read(gene, true_psi, read_len,
                                           overhang_len)
        read_start, read_end = reads_sampled[1]
        category = reads_sampled[2]
        chosen_iso = reads_sampled[3]
        reads_sampled = reads_sampled[0]
        single_read = (reads_sampled[0] + reads_sampled[2],
                       reads_sampled[1] + reads_sampled[2])
        ...
        if any(array(single_read) != 0):
            ...
            if noisify:
                ...
                if (p_ne_loss > 0) and (chosen_iso == 1) and (rand()
                                                              < p_ne_loss):
                    ...
                    if reads_sampled[1] == 1:
                        ...
                        continue
                if (p_ne_gain > 0) and (chosen_iso == 1) and (rand()
                                                              < p_ne_gain):
                    if reads_sampled[1] == 1:
                        # Append read twice
...
def simulate_reads(gene, true_psi, num_reads, read_len, overhang_len):
    ...
    if type(true_psi) != list:
        raise Exception(
            "simulate_reads: expects true_psi to be a probability vector summing to 1."
        )
    if len(gene.isoforms) == 2:
        raise Exception(
            "simulate_reads: should use simulate_two_iso_reads for genes with only two isoforms."
        )
    if sum(true_psi) != 1:
...
def sample_binomial_frag_len(frag_mean=200, frag_variance=100):
    ...
    if frag_variance < frag_mean:
        ...
        print("Sampling frag_mean=", frag_mean, " frag_variance=",
              frag_variance)
        print("r: ", r, "  p: ", p)
...
def simulate_paired_end_reads(gene,
                              true_psi,
                              num_reads,
                              read_len,
                              overhang_len,
    ...
    for k in range(0, num_reads):
        ...
        while insert_len < 0:
            if bino_sampling:
                frag_len = sample_binomial_frag_len(
                    frag_mean=mean_frag_len, frag_variance=frag_variance)
            else:
                frag_len = sample_normal_frag_len(frag_mean=mean_frag_len,
                                                  frag_variance=frag_variance)
            insert_len = frag_len - (2 * read_len)
            if insert_len < 0:
                raise Exception(
                    "Sampled fragment length that is shorter than 2 * read_len!"
                )
                #print "Sampled fragment length that is shorter than 2 * read_len!"
            sampled_frag_lens.append(frag_len)
        reads_sampled = sample_random_read_pair(gene, true_psi, read_len,
                                                overhang_len, insert_len,
                                                mean_frag_len)
        alignment = reads_sampled[0]
...
def sample_random_read_pair(gene, true_psi, read_len, overhang_len, insert_len,
                            mean_frag_len):
    """
    ...
    isoform_position_probs = compute_read_pair_position_prob(
        iso_len, read_len, frag_len)
    # sanity check
    ...
    pe_read = (genomic_left_read_start, genomic_left_read_end,
               genomic_right_read_start, genomic_right_read_end)
    alignment, frag_lens = gene.align_read_pair(pe_read[0],
                                                pe_read[1],
                                                pe_read[2],
                                                pe_read[3],
...
def sample_random_read(gene, true_psi, read_len, overhang_len):
    ...
    # Choose isoform to sample read from
    chosen_iso = list(multinomial(1, psi_frags)).index(1)
    isoform_position_prob = ones(
        num_positions[chosen_iso]) * iso_probs[chosen_iso]
    ...
    genomic_read_start, genomic_read_end = \
                        gene.isoforms[chosen_iso].isoform_coords_to_genomic(sampled_read_start,
                                                                            sampled_read_end)
    alignment, category = gene.align_read(genomic_read_start,
                                          genomic_read_end,
                                          overhang=overhang_len)
    return (tuple(alignment), [sampled_read_start,
                               sampled_read_end], category, chosen_iso)
