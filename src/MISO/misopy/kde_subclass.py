...
def plotkde(covfact):
    ...
    plt.plot(ind,
             alpha * stats.norm.pdf(ind, loc=mlow) +
             (1 - alpha) * stats.norm.pdf(ind, loc=mhigh),
             color="r",
             label='DGP: normal mix')
...
if __name__ == '__main__':
    ...
    xn = np.concatenate([
        mlow + np.random.randn(alpha * n_basesample), mhigh + np.random.randn(
            (1 - alpha) * n_basesample)
    ])
