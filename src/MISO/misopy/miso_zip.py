...
class MISOCompressor:
    ...
    def uncompress(self, compressed_filename, output_dir):
        ...
        print("Uncompressing %s into %s" % (compressed_filename, output_dir))
...
def compress_miso(output_filename, input_dirs, comp_ext=".misozip"):
    """
...
def unzipper(zip_file, outdir):
    ...
    """
    zf = zipfile.ZipFile(zip_file, "r", allowZip64=True)
...
def main():
    ...
    if (options.compress is None) and (options.uncompress is None):
        ...
        greeting()
        print(
            "Error: Cannot process --compress and --uncompress at same time.")
        sys.exit(1)
